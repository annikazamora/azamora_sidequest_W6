...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerImg = await loadImageAsync("assets/cow-sheet.png");
  const boarImg = await loadImageAsync("assets/boarSpriteSheet.png");
  const leafImg = await loadImageAsync("assets/leaf.png");
...
async function loadBackgrounds(levelPkg) {
  ...
  return {
    ...
    bgExtra: await loadImageAsync("assets/background_layer_4.png"), // Added fourth layer
  };
}
